...
instrumentation needed to manage it. We propose a client architecture built on
two repositories. A content repository preserves the AI session data as the
system of record. A metadata repository derives from it the information an
organization needs to manage its AI: an inventory of AI systems and instances,
their utilization and performance, an explanation of what the AI investment
bought, and the metrics from which return on investment is computed, alongside
the quantitative and qualitative measures of utilization and effectiveness
(KPIs, MOEs, MOPs). The same captured record is the substrate required to
detect, measure, and mitigate hallucinations as they emerge. We argue that AI
...
In this paper we propose a client architecture built on two repositories. A
content repository preserves the AI session data itself, the prompts, turns, tool
calls, and context of each interaction, as the organization's system of record. A
metadata repository derives from that content the information an organization needs
to manage its AI: an inventory of the AI systems in use, their utilization and
performance, an explanation of what the AI investment bought, and the metrics from
which return on investment is computed. The AI systems in use are instrumented,
their session content is stored in the content repository, metadata is derived into
the metadata repository and segregated there by sensitivity, and outcomes, risks,
and return are drawn from the metadata repository and reported to the organization.
The architecture is proposed for development and adoption by a sponsoring
organization, \sponsor{}, whose deployment data will populate the inventory,
utilization, and return measures the metadata repository is built to produce. We argue
...
of record. Derived metadata is held in the metadata repository, segregated by
sensitivity into an inventory, utilization, performance, and return store, a
hallucination-and-risk store, and a session-continuity store. Outcomes, risks, and
return are drawn into the outcome and risk store and presented by reporting. Every
record is attributable by organization, product line, and purpose.}
\label{fig:arch}
\end{figure}
...
The content repository holds the session data itself: the system and
...
store, which carries the inventory of AI systems, their utilization and
performance, and the return-on-investment measures of
Section~\ref{sec:measurement}, is one part. Hallucination and risk
