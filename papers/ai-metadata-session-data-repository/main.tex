% AI Metadata and Session Data Repository -- Whitepaper
% Genre: forward-looking proposal / design / position paper.
% MOOD RULE: predictive/design mood only. No evidentiary claims ("we measured",
% "results show"). Prior internal work is INCORPORATED as informed design, never
% CITED as evidence. External prior art (standards, observability, pharmacovigilance)
% is cited freely. Voice: no em-dashes; no filler adjectives; active voice.
% Class: article for portability. Swap to {neus2025} only for a conference version.
...
% Author: Johnny Morgan, sole author (confirmed 2026-08-08).
...
\begin{abstract}
% DRAFT stub in predictive mood -- rewrite in author voice.
...
% seven-layer architecture, its immutable-event data model, a measurement
...
% Dr. Song 5-paragraph format. Predictive/design mood; external cites only.
...
\label{sec:problem}
% MOOD: diagnostic / predictive. Funnels into Section 3.
...
% MOOD: argument (predictive/normative). Descriptive-leads: the epistemic claim
% first, the enterprise mandate as its conclusion. External cites only.
...
\label{sec:related}
% MOOD: survey / positioning. External cites only.
...
% MOOD: design. SIMPLE top-level flow; complexity is a drill-down.
...
\section{Data Model}
\label{sec:datamodel}
% MOOD: design. Draft prose below; iterate for voice.
...
% MOOD: design/predictive. Measures are EXOGENOUS + LAYERED (org-defined); the
% architecture collects/detects/catalogs/stores OUTCOMES for portfolio reporting.
% Figure F3.
...
\label{sec:risk}
% MOOD: design/predictive. Broadened from "Risk Lifecycle" to cover both signs.
% Adverse-event lifecycle kept as the harm machinery. Figure F4.
...
% MOOD: design/normative. The requested "sample set" of obligation classes.
...
\label{sec:trust}
% MOOD: design/normative. Central tenet: calibrated trust, earned + curated.
...
\label{sec:operation}
% MOOD: design. Main end-to-end operation still to draft:
% instrumentation points (API gateway/proxy, agent hooks, chat/IDE), ingestion,
% three-tier build, query/reporting surface, tenancy, data residency. Figure F5.
...
% MOOD: roadmap / predictive (explicitly future work).
...
% MOOD: explicit falsifiable predictions, dated.
...
% MOOD: design/normative. External cites only.
